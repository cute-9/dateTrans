...
<template>
  <div>
    ...
    <!-- <Button type="info" style="width: 100%" @click="handleSaveChart"
      >保存</Button
    > -->
  </div>
</template>
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const form = JSON.parse(JSON.stringify(this.formInline))
          for (const key in this.titleForm) {
