...
export default {
  ...
  methods: {
    ...
    numMap(name) {
      ...
      const matchNum = JSON.parse(JSON.stringify(this.matchOptions))
      const tableData = JSON.parse(JSON.stringify(this.tableData))
      const nameNum = tableData.map(item => {
        return item.name
      })
      // 统计字段名出现的次数
      const obj = {}
      ...
      for (const key in obj) {
        if (obj[key] > 2) {
          ...
          if (key === this.hightName) {
            const indexs = tableData.find(currentValue => {
    ...
    change() {
      ...
      listMapping({
        ...
        if (responseCode === 200) {
          ...
          const obj = deep(data, 'properties')
          for (const key in obj) {
            this.options.push({
              value: key,
              label: key
